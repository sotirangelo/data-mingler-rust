...
async fn it_loads_the_dvm() -> Result<(), Box<dyn std::error::Error + 'static>> {
    ...
    let load_dvm_output = Command::new("cargo")
        .arg("run")
        .arg("--bin")
        .arg("dvm-to-neo4j")
        .arg("--")
        ...
        .arg("test_data/example_dvm.xml")
        .output()
        .expect("failed to execute process");

    if !load_dvm_output.status.success() {
        eprintln!(
            "stdout: {}",
            String::from_utf8_lossy(&load_dvm_output.stdout)
        );
        eprintln!(
            "stderr: {}",
            String::from_utf8_lossy(&load_dvm_output.stderr)
        );
        process::exit(1);
    }
